...
PhotonAnalyzer::Process (const edm::Event & iEvent, TClonesArray * rootPhotons, const edm::EventSetup& iSetup)
{
  ...
  if (verbosity_ > 1)
    ...
    for (unsigned int j = 0; j < nPhotons; j++)
    {
      ...
      cat::Photon localPhoton (photon->px (), photon->py (), photon->pz (), photon->energy ());
      ...
      double chIso = patPhoton->chargedHadronIso();
      double phIso = patPhoton->photonIso();
      double nhIso = patPhoton->neutralHadronIso();
      double puChIso = patPhoton->puChargedHadronIso();
      ...
      new ((*rootPhotons)[j]) cat::Photon (localPhoton);
      if (verbosity_ > 2)
