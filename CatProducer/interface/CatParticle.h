#ifndef CatParticle_h
#define CatParticle_h
...
namespace cat
{
	class CatParticle : public TLorentzVector
	{
	...
	public:
		...
		CatParticle(const CatParticle& particle) :
		...
		CatParticle(Double_t px, Double_t py, Double_t pz, Double_t e) :
		...
		CatParticle(Double_t px, Double_t py, Double_t pz, Double_t e, Double_t vtx_x, Double_t vtx_y, Double_t vtx_z) :
			TLorentzVector(px,py,pz,e)
		...
		CatParticle(Double_t px, Double_t py, Double_t pz, Double_t e, Double_t vtx_x, Double_t vtx_y, Double_t vtx_z,Int_t type, Float_t charge) :
			TLorentzVector(px,py,pz,e)
			,vertex_(vtx_x,vtx_y,vtx_z)
			,type_(type)
			,charge_(charge)
		...
		CatParticle(const TLorentzVector &momentum) :
		...
		CatParticle(const TLorentzVector &momentum, const TVector3 &vertex) :
			TLorentzVector(momentum)
			,vertex_(vertex)
			,type_(0)
			,charge_(0.)
		...
		CatParticle(const TLorentzVector &momentum, const TVector3 &vertex, Int_t type, Float_t charge) :
			TLorentzVector(momentum)
			,vertex_(vertex)
			,type_(type)
			,charge_(charge)
		...
		virtual TString typeName() const { return "CatParticle"; }
		...
		friend std::ostream& operator<< (std::ostream& stream, const CatParticle& part)
		{
