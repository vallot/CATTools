...
namespace cat {
  ...
  class CATSecVertexV3Producer : public edm::stream::EDProducer<> {
    public:
      explicit CATSecVertexV3Producer(const edm::ParameterSet & iConfig);
      virtual ~CATSecVertexV3Producer() { }
...
cat::CATSecVertexV3Producer::CATSecVertexV3Producer(const edm::ParameterSet & iConfig) :
  ...
  vertexLabel_(consumes<reco::VertexCollection>(iConfig.getParameter<edm::InputTag>("vertexLabel")))
{
  ...
  void
cat::CATSecVertexV3Producer::produce(edm::Event & iEvent, const edm::EventSetup & iSetup)
{
  ...
  for(VertexCompositePtrCandidateCollection::const_iterator sv = secvtxSrc->begin() ; sv != secvtxSrc->end() ; ++sv) {
    ...
    cand1 = sv->daughter(0);
    cand2 = sv->daughter(1);
    if ( cand1->pdgId() * cand2->pdgId() != -169  && cand1->pdgId()*cand2->pdgId() != -121) continue;

    //const reco::TransientTrack  transTrack1, transTrack2;
    if ( cand1->get<TrackRef>().isNull() || cand2->get<TrackRef>().isNull()) continue;
    auto transTrack1 = trackBuilder->build(cand1->get<TrackRef>());
    auto transTrack2 = trackBuilder->build(cand2->get<TrackRef>());
    fitTransientTracks(out_, pv, transTrack1, transTrack2, std::abs(cand1->pdgId()) );
  }
...
void cat::CATSecVertexV3Producer::fitTransientTracks(cat::SecVertexV2Collection* out_, reco::Vertex& goodPV, reco::TransientTrack& ttrack1, reco::TransientTrack& ttrack2, int pdgId)
{
  ...
  FreeTrajectoryState ipStatePos = ttrack1.impactPointTSCP().theState();
  FreeTrajectoryState ipStateNeg = ttrack2.impactPointTSCP().theState();
  ...
  if (applyCuts_ && ( !transVertex.isValid() or transVertex.totalChiSquared() < 0. )) return;

  const reco::Vertex vertex = transVertex;
  if ( vertex.normalizedChi2() > cut_vertexChi2_ ) return;
...
using namespace cat;
DEFINE_FWK_MODULE(CATSecVertexV3Producer);
