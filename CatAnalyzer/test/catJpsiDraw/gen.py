...
genParticleHandle, genParticleLabel = Handle("vector<reco::GenParticle>"), "prunedGenParticles"
...
diff_gen = ROOT.TH1F("diff_gen","diff eff. of gen J/#psiper pT; pT(GeV/c) ; Entries",100,0,100)
diff_1   = ROOT.TH1F("diff_eff1","Differential Eff. of J/#psi reco module #1 per pT ; pT(GeV/c) ; Entries",100,0,100)
diff_3   = ROOT.TH1F("diff_eff3","Differential Eff. of J/#psi reco module #3 per pT ; pT(GeV/c) ; Entries",100,0,100)
diff_40  = ROOT.TH1F("diff_eff40","Differential Eff. of J/#psi reco module #40 per pT ; pT(GeV/c) ; Entries",100,0,100)
diff_41  = ROOT.TH1F("diff_eff41","Differential Eff. of J/#psi reco module #41 per pT ; pT(GeV/c) ; Entries",100,0,100)
...
for i,event in enumerate(events) :
  ...
  for gen in genParticleHandle.product() :
    if ( abs(gen.pdgId()) == 443 and abs(gen.eta())< 2.4 and gen.numberOfDaughters()==2 and (  abs(gen.daughter(0).pdgId())==13 or abs(gen.daughter(0).pdgId()) == 11 ) ) :
      jpsi_idx = jpsi_idx +1 
      genJpsis.append( gen )
      print "genJpsi pt : %f , eta %f, phi %f"%(gen.pt(), gen.eta(), gen.phi())
      diff_gen.Fill( gen.pt())
