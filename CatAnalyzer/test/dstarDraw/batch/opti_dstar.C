int opti_dstar(float relPt_val=0.05, float dR_val =0.15 )
{
  ...
  //for( float dR = 0.025 ; dR <0.5 ; dR = dR+(0.5/20.)) {
  {
      ...
      RooPlot* dstar_mass_frame = x.frame();
      d1->plotOn(dstar_mass_frame);

      model.plotOn(dstar_mass_frame);
      model.plotOn(dstar_mass_frame, RooFit::Components(exp_pdf),RooFit::LineStyle(kDashed));
      model.plotOn(dstar_mass_frame, RooFit::Components(gaus_pdf),RooFit::LineColor(kRed), RooFit::LineStyle(kDashed));
      ...
      dstar_mass_frame->Draw();
      dstar_mass_frame->SetTitle(canvas_name.Data());
