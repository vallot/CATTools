...
namespace cat
{
	...
	public:
		...
		virtual TString typeName() const { return "GenEvent"; }


		friend std::ostream& operator<< (std::ostream& stream, const GenEvent& genEvt)
		{
			stream << "GenEvent - is ";
			...
			//stream << "GenEvent - Charge=" << setw(2) << jet.charge() << " (Et,eta,phi)=("<< setw(10) << jet.Et() <<","<< setw(10) << jet.Eta() <<","<< setw(10) << jet.Phi() << ")"
			return stream;
		};
