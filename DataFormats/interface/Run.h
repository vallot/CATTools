...
namespace cat
{
	...
	public:
		...
		void setHLTinfos(vector<cat::HLTInfo> hltInfos) { hltInfos_ = hltInfos; }

		cat::HLTInfo getHLTinfo(UInt_t RunID) { 
		...
		vector<cat::HLTInfo> copyHLTinfos() const { return hltInfos_; }; // for the skimmer
	...
	private:
		...
		std::string hltInputTag_;							// name of the HLT inputtag provided in the Producer config
