...
namespace cat
{
	...
	public:
		...
		Muon(Double_t px, Double_t py, Double_t pz, Double_t e) :
			Lepton(px,py,pz,e)
			,vetoEm_(-9999.)
			,vetoHad_(-9999.)
			,chi2_(+9999.0)
		...
		Muon(Double_t px, Double_t py, Double_t pz, Double_t e, Double_t vtx_x, Double_t vtx_y, Double_t vtx_z) :
		...
		Muon(Double_t px, Double_t py, Double_t pz, Double_t e, Double_t vtx_x, Double_t vtx_y, Double_t vtx_z, Int_t type, Int_t charge) :
		...
		Muon(const TLorentzVector &momentum) :
			Lepton(momentum)
			,vetoEm_(-9999.)
			,vetoHad_(-9999.)
			,chi2_(+9999.0)
		...
		Muon(const TLorentzVector &momentum, const TVector3 &vertex, Int_t type, Int_t charge) :
...
//		Float_t dBError() const { return dBError_; }
		virtual TString typeName() const { return "Muon"; }
		...
		friend std::ostream& operator<< (std::ostream& stream, const Muon& muon)
		{
			stream << "Muon - Charge=" << muon.charge() << " (Et,eta,phi)=("<< muon.Et() <<","<< muon.Eta() <<","<< muon.Phi() << ")  vertex(x,y,z)=("<< muon.vx() <<","<< muon.vy() <<","<< muon.vz() << ")" << endl
