...
namespace cat
{
	...
	public:
		...
		Jet(Double_t px, Double_t py, Double_t pz, Double_t e, Double_t vtx_x, Double_t vtx_y, Double_t vtx_z) :
		...
		Jet(Double_t px, Double_t py, Double_t pz, Double_t e, Double_t vtx_x, Double_t vtx_y, Double_t vtx_z, Int_t type, Float_t charge) :
		...
		Jet(const TLorentzVector &momentum) :
			Particle(momentum)
			,jetType_(0)
			,nConstituents_(-9999)
			,jetArea_(-9999.)
		...
		Jet(const TLorentzVector &momentum, const TVector3 &vertex, Int_t type, Float_t charge) :
    ...
    Float_t	btag_softPFMuonRetrainedBJetsTags() const { return btag_softPFMuonRetrainedBJetsTags_; }
		...
		virtual TString typeName() const { return "Jet"; }
    ...
    void setBtag_softPFMuonRetrainedBJetsTags(Float_t btag_softPFMuonRetrainedBJetsTags) { btag_softPFMuonRetrainedBJetsTags_ = btag_softPFMuonRetrainedBJetsTags; }
		...
		friend std::ostream& operator<< (std::ostream& stream, const Jet& jet)
		{
			stream << "Jet - Charge=" << setw(2) << jet.charge() << " (Et,eta,phi)=("<< setw(10) << jet.Et() <<","<< setw(10) << jet.Eta() <<","<< setw(10) << jet.Phi() << ")"
